...
function scoreTechnical(hist: Candle[]): { score: number; signals: string[] } {
  ...
  // 1. MA arrangement (max 8) — strict: need close>MA5>MA20>MA60 for full bull
  ...
  score += maScore;
  // Strict labeling: only call it bull arrangement when ALL 4 conditions met
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        // Aggressive thresholds: 75/60/50
        const grade =
          totalScore >= 75 ? '強力買進' :
          totalScore >= 60 ? '買進' :
          totalScore >= 50 ? '觀望' : '偏弱';
