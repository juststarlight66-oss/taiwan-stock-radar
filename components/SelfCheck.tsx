...
/**
 * getScoreGrade — aggressive thresholds aligned with daily scanner:
 *   >= 75  強力買進
 *   >= 60  買進
 *   >= 50  觀望
 *   <  50  偏弱
...
function getScoreGrade(score: number): { label: string; color: string } {
  if (score >= 75) return { label: '強力買進', color: 'text-emerald-300 bg-emerald-500/15 border-emerald-500/40' };
  if (score >= 60) return { label: '買進', color: 'text-sky-300 bg-sky-500/15 border-sky-500/40' };
  if (score >= 50) return { label: '觀望', color: 'text-amber-300 bg-amber-500/15 border-amber-500/40' };
...
export default function SelfCheck() {
  ...
  const suggestions = useMemo(() => {
    ...
    return (allScores?.all_stock_scores ?? []).filter(
      (s) => s.stock_id.startsWith(q) || s.name.toLowerCase().includes(q)
    ).slice(0, 8);
  }, [query, allScores]);
