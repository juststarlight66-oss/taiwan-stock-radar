      ...
      <div className="rounded-xl border border-gray-700/60 bg-gray-900/60 px-5 py-4">
        ...
        <div className="mt-4 pt-4 border-t border-gray-700/40">
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className="text-lg font-bold font-mono text-sky-300">{stock.total_score.toFixed(1)}<span className="text-xs text-gray-600">/100</span></span>
          </div>
          <div className="h-2 rounded-full bg-gray-700/60 overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(stock.total_score, 100)}%` }}
            />
          </div>
        </div>
      </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-700/40 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="rounded-lg bg-gray-800/60 px-2 py-2">
                  <div className="text-[10px] text-gray-500 mb-0.5">進場</div>
                  <div className="text-xs font-mono font-bold text-emerald-400">{stock.strategy.entry ?? '—'}</div>
                </div>
                <div className="rounded-lg bg-gray-800/60 px-2 py-2">
                  <div className="text-[10px] text-gray-500 mb-0.5">目標</div>
                  <div className="text-xs font-mono font-bold text-sky-400">{stock.strategy.target ?? '—'}</div>
                </div>
                <div className="rounded-lg bg-gray-800/60 px-2 py-2">
              ...
              {stock.strategy.recommendation && (
                <div className="text-xs text-gray-400 text-center pt-1">
                  策略：<span className="text-gray-200">{stock.strategy.recommendation}</span>
                </div>
              )}
            </div>
          )}
...
export default function SelfCheck() {
  ...
  function doSearch(ticker: string) {
    ...
    // Fallback: search historical scan files via index.json
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-1 rounded-xl border border-gray-700/60 bg-gray-900 shadow-xl z-30 overflow-hidden">
            {suggestions.map((s) => (
              <button
                key={s.stock_id}
                onMouseDown={() => { setQuery(s.stock_id); doSearch(s.stock_id); }}
              ...
              >
                ...
                <span className={`text-xs font-mono ${getChangeColor(s.change_pct)}`}>
