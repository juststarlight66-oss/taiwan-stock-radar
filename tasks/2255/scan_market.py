...
版本：v7.2 (TWSE+TPEx+ESB 全市場覆蓋含興櫃)
優化項目：
...
def fetch_esb_day_all() -> Dict[str, Dict]:
    """
    從 TPEx ESB OpenAPI 取得興櫃股票當日 OHLCV。
    API: https://www.tpex.org.tw/openapi/v1/tpex_esb_daily_trading_info
    補充上市+上櫃後，再加入興櫃約 ~300 檔，合計覆蓋 ~2,200+ 檔。
    回傳格式與 fetch_stock_day_all() 相同：
    { stock_id: {date, open, high, low, close, volume, change, change_pct, name} }
    """
    today = datetime.now().strftime('%Y%m%d')
    url = 'https://www.tpex.org.tw/openapi/v1/tpex_esb_daily_trading_info'
    print('[API] 呼叫 TPEx ESB 興櫃 tpex_esb_daily_trading_info...')
    try:
        r = _http_get(url, headers=TWSE_HEADERS, timeout=30, verify=False)
        rows = r.json()
    except Exception as e:
        print(f'[API] TPEx ESB 興櫃 OHLCV 失敗（已重試3次）：{e}')
        return {}

    if not isinstance(rows, list) or not rows:
        print('[API] TPEx ESB 興櫃 OHLCV 無資料（可能為非交易日）')
        return {}
    ...
    for row in rows:
        try:
            ...
            close_str = clean_esb(row.get('Close', row.get('ClosingPrice', row.get('收盤價', ''))))
            open_str  = clean_esb(row.get('Open',  row.get('OpeningPrice', row.get('開盤價', ''))))
            high_str  = clean_esb(row.get('High',  row.get('HighestPrice', row.get('最高價', ''))))
            low_str   = clean_esb(row.get('Low',   row.get('LowestPrice',  row.get('最低價', ''))))
            vol_str   = clean_esb(row.get('TradingShares', row.get('TradeVolume', row.get('成交量', row.get('Volume', '')))))
            chg_str   = clean_esb(row.get('Change', row.get('漲跌', '')))
            name      = str(row.get('CompanyName', row.get('Name', row.get('公司名稱', code)))).strip()
            ...
            print(f'[WARN] TPEx ESB 興櫃 OHLCV 列解析失敗：{e}')
            continue

    print(f'[API] TPEx ESB 興櫃 OHLCV 解析完成：{len(result)} 檔')
    return result
...
def run_five_dimension_scan(verbose=True) -> Dict:
    ...
    if tpex_ohlcv:
        ...
        today_ohlcv = merged
    # 合併興櫃 OHLCV（ESB，上市+上櫃已有資料的不覆蓋）
    esb_ohlcv = fetch_esb_day_all()
    if esb_ohlcv:
        before_esb = len(today_ohlcv)
        merged_esb = {**esb_ohlcv, **today_ohlcv}  # 上市/上櫃優先
        added_esb = len(merged_esb) - before_esb
        print(f'[系統] 合併興櫃 OHLCV：{before_esb} + {len(esb_ohlcv)} → {len(merged_esb)} 檔（新增興櫃 {added_esb} 檔）')
        today_ohlcv = merged_esb
    if not today_ohlcv:
        print("[警告] STOCK_DAY_ALL 無資料，嘗試使用快取繼續執行")

    # ── Step 2：取得基本面數據（BWIBBU_ALL 上市 + TPEx peratio 上櫃，單次呼叫）────
    bwibbu_data = fetch_bwibbu_all()
    tpex_bwibbu = fetch_tpex_bwibbu_all()
    if tpex_bwibbu:
        # 上市資料優先（TWSE 覆蓋 TPEx 同代號）
        merged_bwibbu = {**tpex_bwibbu, **bwibbu_data}
        added_bwibbu = len(merged_bwibbu) - len(bwibbu_data)
        print(f'[系統] 合併 BWIBBU：TWSE {len(bwibbu_data)} + TPEx {len(tpex_bwibbu)} → {len(merged_bwibbu)} 檔（新增上櫃 {added_bwibbu} 檔）')
        bwibbu_data = merged_bwibbu

    # ── Step 2b：取得真實籌碼資料（T86 三大法人 + MI_MARGN 融資融券）──
    t86_data    = fetch_t86_chips()
    margin_data = fetch_margin_data()
    print(f"[API] 籌碼資料：T86 {len(t86_data)} 檔三大法人 / MI_MARGN {len(margin_data)} 檔融資融券")
