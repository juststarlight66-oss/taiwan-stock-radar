...
def get_stock_pool(today_ohlcv: Dict[str, Dict] = None) -> Dict[str, str]:
    ...
    # 用 STOCK_DAY_ALL 的 codes 補充（確保涵蓋所有正股）
    if today_ohlcv:
        before = len(pool)
        ...
        after = len(pool)
        if after > before:
            print(f"[股票池] STOCK_DAY_ALL 補充 {after - before} 檔，總計 {after} 檔")
...
def calculate_entry_exit(stock_data, technical) -> Dict:
    """計算進場點、目標價、停損點"""
    close     = stock_data['close']
    ma5       = technical.get('ma5', close)
    entry     = min(ma5 * 1.01, close * 0.98)
    target    = max(close * 1.15, ma5 * 1.2)
...
def determine_strategy(score) -> str:
    """v3 積極型判定：S≥75, A 60-74, B 45-59, C 30-44, D<30"""
    if score >= 75:   return '強力買進 - 重倉佈局'
    elif score >= 60: return '積極買進 - 中型部位'
    elif score >= 45: return '伺機佈局 - 小量試單'
    elif score >= 30: return '減碼觀察'
    else:             return '暫停操作 - 等待訊號'


def get_holding_advice(recommendation: str) -> str:
    """依推薦等級生成持有建議文字。"""
    if '強力買進' in recommendation:
        return '建議等次日開盤後 ±1% 確認動能再進場｜建議持有至 T+5 再分批減碼，核心部位可續抱'
    elif '積極買進' in recommendation:
        return '建議持有至 T+5 再分批減碼，核心部位可續抱'
    elif '伺機佈局' in recommendation:
        return '可小量建立試單，確認方向後再加碼｜持有至 T+3 觀察'
    else:
        return ''
...
def run_five_dimension_scan(verbose=True) -> Dict:
    ...
    for stock_id, name in STOCK_POOL.items():
        ...
        today_data  = hist[-1]
        entry_exit  = calculate_entry_exit(today_data, tech)
    ...
    for i, r in enumerate(top10, 1):
        ...
        lines.append(f"   技術:[{tech_sigs}] 進場:{r['strategy']['entry']} 目標:{r['strategy']['target']} 停損:{r['strategy']['stop_loss']}")
